    	...
    	def naredi_potezo(self, pozicija1, pozicija2):
        	...
        	for i in veljavne_poteze():
        		if (i == (i1, j1, i2, j2)) or (i == (i2, j2, i1, j1)): # Primerja povlečene poteze z veljavnimi.
                		...
                		if stanje == NI_KONEC:
        	 			self.na_potezi = nasprotnik(self.na_potezi) # Igre ni konec, zdaj je na potezi nasprotnik.
        			else:
                			self.na_potezi = None # Igre je konec.
        	 		return stanje
        		elif (self.plosca[i1][j1] != NEPOKRITO) or (self.plosca[i2][j2] != NEPOKRITO) or (self.na_potezi == None): # Neveljavna poteza.
        			return None
        		else:
    	...
    	def minimax(self, globina, maksimiziramo):
        	...
        	if (rezultat == IGRALEC_RED) or (rezultat == IGRALEC_BLUE): # Igre je konec, vrnemo njeno vrednost.
            		if zmagovalec == self.kdo_sem:
                		...
                		return (None, -Minimax.ZMAGA)
        	elif rezultat == NI_KONEC:
            		if globina == 0:
        			return (None, self.vrednost_igre())  # Igre ni konec.
            		else:
